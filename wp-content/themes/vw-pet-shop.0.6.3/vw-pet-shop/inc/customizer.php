...
function vw_pet_shop_customize_register( $wp_customize ) {
	...
	$wp_customize->add_setting('vw_pet_shop_width_option',array(
        'default' => __('Full Width','vw-pet-shop'),
	...
	$wp_customize->add_setting('vw_pet_shop_page_layout',array(
        'default' => __('One Column','vw-pet-shop'),
        'sanitize_callback' => 'vw_pet_shop_sanitize_choices'
	));
	...
	$wp_customize->add_setting('vw_pet_shop_loader_icon',array(
        'default' => __('Two Way','vw-pet-shop'),
        'sanitize_callback' => 'vw_pet_shop_sanitize_choices'
	));
	$wp_customize->add_control('vw_pet_shop_loader_icon',array(
        'type' => 'select',
        'label' => __('Pre-Loader Type','vw-pet-shop'),
        'section' => 'vw_pet_shop_left_right',
        'choices' => array(
            'Two Way' => __('Two Way','vw-pet-shop'),
            'Dots' => __('Dots','vw-pet-shop'),
            'Rotate' => __('Rotate','vw-pet-shop')
        ),
	) );
    ...
    $wp_customize->add_setting('vw_pet_shop_excerpt_settings',array(
        'default' => __('Excerpt','vw-pet-shop'),
        'transport' => 'refresh',
        'sanitize_callback' => 'vw_pet_shop_sanitize_choices'
    ));
	...
	$wp_customize->add_setting( 'vw_pet_shop_metabox_hide_show',array(
      'default' => 1,
      'transport' => 'refresh',
      'sanitize_callback' => 'vw_pet_shop_switch_sanitization'
    ));  
    $wp_customize->add_control( new VW_Pet_Shop_Toggle_Switch_Custom_Control( $wp_customize, 'vw_pet_shop_metabox_hide_show',array(
      'label' => esc_html__( 'Show / Hide Metabox','vw-pet-shop' ),
      'section' => 'vw_pet_shop_responsive_media'
    )));
	...
	//Content Creation
	$wp_customize->add_section( 'vw_pet_shop_content_section' , array(
    	'title' => __( 'Customize Home Page Settings', 'vw-pet-shop' ),
		'priority' => null,
		'panel' => 'vw_pet_shop_panel_id'
	));

	$wp_customize->add_setting('vw_pet_shop_content_creation_main_control', array(
		'sanitize_callback' => 'esc_html',
	));

	$homepage= get_option( 'page_on_front' );

	$wp_customize->add_control(	new VW_Pet_Shop_Content_Creation( $wp_customize, 'vw_pet_shop_content_creation_main_control', array(
		'options' => array(
			esc_html__( 'First select static page in homepage setting for front page.Below given edit button is to customize Home Page. Just click on the edit option, add whatever elements you want to include in the homepage, save the changes and you are good to go.','vw-pet-shop' ),
		),
		'section' => 'vw_pet_shop_content_section',
		'button_url'  => admin_url( 'post.php?post='.$homepage.'&action=edit'),
		'button_text' => esc_html__( 'Edit', 'vw-pet-shop' ),
	)));
...
final class VW_Pet_Shop_Customize {
	...
	public function sections( $manager ) {
		...
		// Register sections.
		$manager->add_section(new VW_Pet_Shop_Customize_Section_Pro($manager,'example_1', array(
			'priority'   => 1,
		...
		// Register sections.
		$manager->add_section(new VW_Pet_Shop_Customize_Section_Pro($manager,'example_2', array(
			'priority'   => 1,
